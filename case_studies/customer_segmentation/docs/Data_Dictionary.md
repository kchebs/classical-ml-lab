...
The segmentation case study uses two demographics-style tables (same columns):

- AZDIAS_Subset.csv: general population sample (full licensed extracts are large and not redistributed here; use `scripts/bootstrap_data.py` for smoke CSVs)
- CUSTOMERS_Subset.csv: mail-order customer sample (same schema; smoke or licensed extract)
